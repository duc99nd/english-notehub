---
id: b2-15-review-unit3
slug: b2-15-review-unit3
titleEn: 'Review Unit 3'
titleVi: 'Ôn tập Unit 3'
subtitleEn: 'Consolidate your mastery of cultural nuances, complex structures, editorial reading, and formal correspondence'
subtitleVi: 'Củng cố khả năng làm chủ sắc thái văn hóa, cấu trúc phức, đọc xã luận và thư tín trang trọng'
level: B2
unit: 3
skill: review
order: 15
cefr: B2
minutes: 10
tags: [review, culture, grammar, reading, writing, consolidation, B2]
audioScript: |
  Unit 3 completes your B2 journey, integrating cultural awareness, complex grammar, analytical reading, and formal writing.
  You now possess the complete range of B2 competencies for professional English.
  This review consolidates your mastery across all three units.
listeningEnabled: true
flashcardCount: 10
quiz:
  - type: choice
    prompt: What is a characteristic of high-context cultures?
    options:
      - 'Direct communication is preferred'
      - 'Meaning is conveyed through context and relationships'
      - 'Written contracts are always required'
    answer: 'Meaning is conveyed through context and relationships'
  - type: choice
    prompt: Which sentence uses a non-defining relative clause correctly?
    options:
      - 'The report that was published highlights key trends.'
      - 'The report, which was published, highlights key trends.'
      - 'The report which was published highlights key trends.'
    answer: 'The report, which was published, highlights key trends.'
  - type: choice
    prompt: What does "in light of" mean in formal writing?
    options:
      - 'Inside a building'
      - 'Considering or taking into account'
      - 'Because of bright light'
    answer: 'Considering or taking into account'
  - type: choice
    prompt: Which phrase is appropriate for a formal email closing?
    options:
      - 'Catch you later!'
      - 'I look forward to your response.'
      - 'Thanks a bunch!'
    answer: 'I look forward to your response.'
  - type: fill
    prompt: 'Please find ___ the requested documentation.'
    answer: 'attached'
---

# Ôn tập Unit 3

## Tóm tắt Unit 3

Unit 3 hoàn thành hành trình B2 của bạn, tích hợp nhận thức văn hóa, ngữ pháp phức tạp, đọc phân tích và viết trang trọng. Bạn hiện sở hữu toàn bộ phạm vi năng lực B2 cho tiếng Anh chuyên nghiệp.

## Tổng hợp các Khái niệm Chính

### 1. Những sắc thái văn hóa (B2-11)

Giao tiếp liên văn hóa đòi hỏi sự nhạy cảm và thích ứng. Hiểu sự khác biệt giữa văn hóa ngữ cảnh cao và thấp giúp bạn điều hướng các mối quan hệ kinh doanh quốc tế.

|| Khái niệm | Ứng dụng |
|| --- | --- |
|| High-context | Giao tiếp gián tiếp, tập trung vào quan hệ |
|| Low-context | Giao tiếp trực tiếp, tập trung vào công việc |
|| Building rapport | Xây dựng mối quan hệ trước khi kinh doanh |
|| Being mindful of | Nhận thức và tôn trọng sự khác biệt |

### 2. Cấu trúc câu phức (B2-12)

Ngữ pháp nâng cao cho phép bạn diễn đạt ý phức tạp một cách tinh vi:

- **Mệnh đề quan hệ không giới hạn**: `The report, which was published, highlights...`
- **Mệnh đề quan hệ rút gọn**: `The data analyzed showed...`
- **Phân từ hoàn thành**: `Having reviewed the data, we concluded...`
- **Mệnh đề danh từ**: `What concerns us most is...`

### 3. Đọc xã luận và quan điểm (B2-13)

Đọc phân tích đòi hỏi khả năng nhận diện cấu trúc lập luận:

- **Claim** (luận đề chính)
- **Evidence** (bằng chứng)
- **Counterargument** (phản luận)

### 4. Thư tín trang trọng (B2-14)

Register trang trọng trong email kinh doanh:

- "I am writing to follow up..."
- "Please find attached..."
- "I look forward to your response at your earliest convenience."

## Hành trình B2: Vòng cung Hoàn chỉnh

### Unit 1: Nền tảng

- Chiến lược đàm phán nâng cao (vocab)
- Ngữ pháp giải quyết xung đột (grammar)
- Trình bày quyết định kiến trúc (reading)

### Unit 2: Ứng dụng Chuyên nghiệp

- Điều phối cuộc họp (vocab)
- Ngữ pháp thuyết phục (grammar)
- Đọc bài báo kinh doanh (reading)
- Viết đề xuất (writing)
- Kể chuyện qua dữ liệu (vocab)
- Câu điều kiện hỗn hợp nâng cao (grammar)

### Unit 3: Tích hợp Nâng cao

- Những sắc thái văn hóa (vocab)
- Cấu trúc câu phức (grammar)
- Đọc xã luận và quan điểm (reading)
- Thư tín trang trọng (writing)

## Đánh giá Mức độ Làm chủ

|| Kỹ năng | Bạn có thể... | Tự đánh giá (1-5) |
|| --- | --- | --- |
|| Đàm phán | Sử dụng thành ngữ đàm phán nâng cao? | |
|| Xung đột | Dùng câu bị động và điều kiện đảo ngữ? | |
|| Kiến trúc | Đọc và thảo luận bài thuyết trình kỹ thuật? | |
|| Điều phối | Dẫn dắt cuộc họp chuyên nghiệp? | |
|| Thuyết phục | Cân bằng hedging và nhấn mạnh? | |
|| Đọc kinh doanh | Phân tích bài báo kinh doanh? | |
|| Đề xuất | Viết đề xuất trang trọng? | |
|| Dữ liệu | Kể chuyện qua dữ liệu? | |
|| Điều kiện | Dùng câu điều kiện hỗn hợp? | |
|| Văn hóa | Điều hướng khác biệt văn hóa? | |

## Hoàn thành B2

Bạn đã hoàn thành toàn bộ chương trình B2. Năng lực tiếng Anh của bạn bao gồm:

- **Từ vựng chuyên nghiệp** trên nhiều lĩnh vực công việc
- **Ngữ pháp phức tạp** cho diễn đạt tinh vi
- **Đọc phân tích** cho văn bản kỹ thuật và chuyên nghiệp
- **Viết trang trọng** cho email, đề xuất và thư tín
- **Nhận thức văn hóa** cho bối cảnh quốc tế

---

_Đây là kết thúc chương trình B2. Tiếp tục thực hành các kỹ năng này trong các bối cảnh chuyên nghiệp thực tế để duy trì sự làm chủ của bạn._
