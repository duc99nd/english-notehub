---
id: a2-05-review-unit1
slug: a2-05-review-unit1
titleEn: Review Unit 1
titleVi: Ôn tập Unit 1
subtitleEn: Ôn tập hỏi giúp đỡ, thói quen hàng ngày, so sánh hơn và sự kiện quá khứ
subtitleVi: Ôn tập hỏi giúp đỡ, thói quen hàng ngày, so sánh hơn và sự kiện quá khứ
level: A2
unit: 1
skill: review
order: 5
cefr: A2
minutes: 10
tags: [review, unit1, grammar, vocabulary]
audioScript: |
  Hãy ôn tập những gì chúng ta đã học trong Unit 1.
  Bạn có thể hỏi giúp đỡ một cách lịch sự.
  Bạn có thể nói về thói quen hàng ngày.
  Bạn có thể so sánh sự vật.
  Bạn có thể nói về các sự kiện quá khứ.
listeningEnabled: true
flashcardCount: 12
quiz:
  - type: choice
    prompt: Làm thế nào để hỏi giúp đỡ một cách lịch sự?
    options:
      - 'Give me help.'
      - 'Could you help me, please?'
      - 'Help me now!'
    answer: 'Could you help me, please?'
  - type: fill
    prompt: 'I wake ___ at six every morning.'
    answer: 'up'
  - type: choice
    prompt: Hình thức so sánh nào đúng?
    options:
      - 'more beautiful'
      - 'beautifuller'
      - 'beautfuller'
    answer: 'more beautiful'
  - type: fill
    prompt: 'Yesterday, she ___ to the market. (go)'
    answer: 'went'
  - type: choice
    prompt: Câu nào ở thì quá khứ đơn?
    options:
      - 'I work at home.'
      - 'I am working.'
      - 'I worked late.'
    answer: 'I worked late.'
---

# Ôn tập Unit 1

## Những gì chúng ta đã học

Trong Unit 1, bạn đã học bốn chủ đề quan trọng:

1. **Hỏi xin giúp đỡ** - Các cụm từ lịch sự để yêu cầu hỗ trợ
2. **Thói quen hàng ngày** - Thì hiện tại đơn cho thói quen
3. **So sánh hơn** - So sánh sự vật với -er và more
4. **Sự kiện trong quá khứ** - Thì quá khứ đơn cho các hành động đã hoàn thành

## Ôn nhanh

### Hỏi xin giúp đỡ

| Tiếng Anh                  | Khi nào dùng          |
| -------------------------- | --------------------- |
| Could you help me?         | Yêu cầu lịch sự       |
| Would you mind helping me? | Rất lịch sự           |
| Sorry to bother you        | Xin lỗi trước khi hỏi |

### Thói quen hàng ngày

- Dùng **thì hiện tại đơn** cho thói quen
- Thêm **-s/-es** với he, she, it
- Từ chỉ thời gian: every day, usually, always

### So sánh hơn

- Tính từ ngắn: tính từ + **-er** + than
- Tính từ dài: **more** + tính từ + than
- Bất quy tắc: good → better, bad → worse

### Sự kiện trong quá khứ

- Động từ có quy tắc: thêm **-ed**
- Động từ bất quy tắc: ghi nhớ các hình thức
- Was/Were cho quá khứ của "to be"

## Bài tập thực hành

Hãy làm bài quiz bên dưới để kiểm tra kiến thức của bạn. Chúc may mắn!

## Bài học tiếp theo

Tiếp tục với **A2-06-email-basics** để học cách viết email cơ bản bằng tiếng Anh.
