---
{
  "id": "b1-06-work-routines",
  "slug": "b1-06-work-routines",
  "titleEn": "Work & Daily Routines",
  "titleVi": "Công việc & Thói quen hàng ngày",
  "subtitleEn": "Nói về công việc, nơi làm việc và các hoạt động công việc hàng ngày",
  "subtitleVi": "Nói về công việc, nơi làm việc và các hoạt động công việc hàng ngày",
  "level": "B1",
  "unit": 2,
  "skill": "vocab",
  "order": 6,
  "cefr": "B1",
  "minutes": 5,
  "tags": [
    "work",
    "job",
    "office",
    "workplace",
    "daily-routine",
    "career"
  ],
  "audioScript": "Tôi thường bắt đầu làm việc lúc 9 giờ sáng và kết thúc lúc 6 giờ tối.\nChiều nay tôi có cuộc họp với quản lý của tôi.\nTôi cần hoàn thành báo cáo này trước thứ Sáu.\n",
  "listeningEnabled": true,
  "flashcardCount": 8,
  "quiz": [
    {
      "type": "choice",
      "prompt": "Deadline nghĩa là gì?",
      "options": [
        "Một loại ghế văn phòng",
        "Thời hạn phải hoàn thành công việc",
        "Một phòng họp"
      ],
      "answer": "Thời hạn phải hoàn thành công việc"
    },
    {
      "type": "fill",
      "prompt": "Tôi cần ___ tài liệu này trước cuộc họp.",
      "answer": "review"
    },
    {
      "type": "choice",
      "prompt": "Từ nào nghĩa là 'quản lý một nhóm'?",
      "options": [
        "To work",
        "To supervise",
        "To vacation"
      ],
      "answer": "To supervise"
    }
  ]
}
---

# Công việc & Thói quen hàng ngày

## Từ vựng công việc thiết yếu

Khi nói về công việc của bạn, bạn cần các từ cho các khía cạnh khác nhau của cuộc sống làm việc.

> **Mẫu câu**: `I work as a [profession]. I usually start at [time].`

## Nghề nghiệp

|                   | Tiếng Anh            | Tiếng Việt |
| ----------------- | -------------------- | ---------- |
| manager           | quản lý              |
| employee          | nhân viên            |
| colleague         | đồng nghiệp          |
| boss / supervisor | sếp / người giám sát |
| freelancer        | người làm tự do      |
| intern            | thực tập sinh        |
| consultant        | chuyên gia tư vấn    |

## Các nơi trong công ty

|              | Tiếng Anh           | Tiếng Việt |
| ------------ | ------------------- | ---------- |
| office       | văn phòng           |
| meeting room | phòng họp           |
| cubicle      | góc làm việc        |
| reception    | quầy lễ tân         |
| break room   | phòng nghỉ giải lao |
| desk         | bàn làm việc        |

## Các hoạt động công việc

|                        | Tiếng Anh         | Tiếng Việt |
| ---------------------- | ----------------- | ---------- |
| to attend a meeting    | tham dự cuộc họp  |
| to meet a deadline     | đáp ứng thời hạn  |
| to review documents    | xem lại tài liệu  |
| to give a presentation | thuyết trình      |
| to handle clients      | xử lý khách hàng  |
| to collaborate         | cộng tác          |
| to delegate            | ủy thác công việc |

## Các cụm từ công việc hữu ích

> "I usually start work at 9 AM." — Tôi thường bắt đầu làm việc lúc 9 giờ sáng.
> "I have a meeting with my manager this afternoon." — Chiều nay tôi có cuộc họp với quản lý.
> "I need to finish this report by Friday." — Tôi cần hoàn thành báo cáo này trước thứ Sáu.
> "Could you help me with this task?" — Bạn có thể giúp tôi với công việc này không?
> "I'll get back to you on that." — Tôi sẽ phản hồi bạn về việc đó.

## Lịch làm việc

- `full-time` — toàn thời gian
- `part-time` — bán thời gian
- `shift work` — làm việc theo ca
- `overtime` — tăng ca
- `remote work` — làm việc từ xa
- `hybrid` — kết hợp (văn phòng + từ xa)

## Nghe và lặp lại

Nhấn nút play để nghe audio. Sau đó lặp lại từng câu thành tiếng.

## Bài tập trắc nghiệm

Trả lời ba câu hỏi trong phần quiz để hoàn thành bài học này.

## Bài học tiếp theo

Ở bài **B1-07-modals-obligation**, bạn sẽ học cách sử dụng động từ khuyết thiếu để diễn tả nghĩa vụ và lời khuyên trong công việc.
