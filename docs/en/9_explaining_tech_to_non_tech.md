...
1.  **Skip the Jargon (Bỏ qua thuật ngữ phức tạp):** Do not mention _Docker, API, payload, garbage collection, query execution plan_ unless you explain what they mean first.
...
- _"**Docker is a tool that** packages our software so it runs the same way on every computer."_
- _"**An API is like a bridge that** allows two different systems to talk to each other."_
- _"**A migration is a script that** updates the structure of our database safely."_

### 4.2. Softening Risk with Modal Verbs

When warning non-technical stakeholders about technical risks, use modals like _might, could,_ or _may_ to sound realistic and professional without causing panic.

- _"If we don't fix this now, it **could lead to** security vulnerabilities down the road."_
- _"Without caching, the database **might crash** under heavy traffic."_
- _"If we rush this feature, we **may introduce** unexpected bugs into production."_
...
- **Tech-focused:** _"The database query returned an empty set."_
- **User-focused:** _"The user **won't see** any search results on the screen."_
- **Tech-focused:** _"The server returned a 500 error due to thread exhaustion."_
- **User-focused:** _"The user **will see** an error screen when they try to save their profile."_
...
- **PM:** _"Why is the user history page loading so slowly? Is the database broken?"_
- **Dev:** _"No, the database is healthy. To put it simply, our database is like a library with millions of books, but we haven't created an index for the user history section yet. The librarian has to check every single book to find the user's history."_
- **PM:** _"Ah, I see. How do we fix that?"_
- **Dev:** _"We just need to create an index. It's like adding a catalog to the library. I will write a migration script to set this up today. Once it's deployed, the page should load instantly."_
- **PM:** _"Perfect! Let's get that done."_
- **Dev:** _"Great. I'll make sure it's tested on staging first to ensure there's zero impact on live users."_
...
- **Using the word "Simple" or "Easy":**
  - Do not say: _"It's very simple, you just configure a Docker container."_ If it were simple to them, they would be programmers. It can sound patronizing. Instead, say: _"We can set up a tool to handle this automatically."_
...
- **Sounding Annoyed:**
  - When a PM asks _"Why does this take three days?"_, do not respond defensively. Explain the hidden steps (e.g., testing, database migrations, security checks) using everyday language.
- **Talking About the Implementation Rather Than the Result:**
  - PMs care about _what happens to the user_. Instead of saying _"We are writing a regex to validate string length on the backend"_ say _"We are ensuring that passwords are long enough so users' accounts are secure."_
