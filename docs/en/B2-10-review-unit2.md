---
{
  'id': 'b2-10-review-unit2',
  'slug': 'b2-10-review-unit2',
  'titleEn': 'Review Unit 2',
  'titleVi': 'Ôn tập Unit 2',
  'subtitleEn': 'Review meeting facilitation, persuasion, and conditionals',
  'subtitleVi': 'Ôn tập điều phối họp, thuyết phục và câu điều kiện',
  'level': 'B2',
  'unit': 2,
  'skill': 'review',
  'order': 10,
  'cefr': 'B2',
  'minutes': 10,
  'tags': ['review', 'unit2', 'B2', 'meetings', 'grammar'],
  'audioScript': "Let's review Unit 2 of B2.\nYou've learned how to facilitate meetings.\nYou can now use advanced conditionals and persuasive grammar.\n",
  'listeningEnabled': true,
  'flashcardCount': 16,
  'quiz':
    [
      {
        'type': 'choice',
        'prompt': 'What does "kick off" mean?',
        'options': ['To end a meeting', 'To start a meeting', 'To cancel a meeting'],
        'answer': 'To start a meeting',
      },
      { 'type': 'fill', 'prompt': 'If I ___ (know), I would have helped.', 'answer': 'had known' },
      {
        'type': 'choice',
        'prompt': 'Which phrase means "to strengthen"?',
        'options': ['Weaken', 'Bolster', 'Delay'],
        'answer': 'Bolster',
      },
      {
        'type': 'fill',
        'prompt': 'Not only ___ it reduce costs, but it also improves performance.',
        'answer': 'does',
      },
      {
        'type': 'choice',
        'prompt': 'ROI stands for:',
        'options': ['Return on Investment', 'Rate of Income', 'Risk of Implementation'],
        'answer': 'Return on Investment',
      },
    ],
}
---

# Review Unit 2

## What we learned

In Unit 2, you built on your B1 foundation with five advanced skills:

1. **Meeting Facilitation** — leading meetings with confidence
2. **Persuasive Grammar** — using grammar to convince
3. **Business Article Reading** — analyzing market reports
4. **Proposal Writing** — structuring compelling proposals
5. **Data Storytelling** — presenting insights effectively

Plus: **Advanced Mixed Conditionals** for hypothetical situations.

## Quick review

### Meeting facilitation phrases

| Phrase          | Use               |
| --------------- | ----------------- |
| kick off        | Start the meeting |
| park that topic | Defer to later    |
| tiebreaker      | Resolve deadlock  |
| draw to a close | End the meeting   |

### Persuasive grammar patterns

- **Cleft**: What matters is the outcome.
- **Inversion**: Not only does it reduce costs, but it also improves quality.
- **Double comparative**: The more we test, the better the result becomes.

### Conditionals

| Type  | Structure                      | Use                    |
| ----- | ------------------------------ | ---------------------- |
| Third | If + had + pp, would have + pp | Past hypothetical      |
| Mixed | past + present                 | Connecting time frames |

## Practice

Answer the five quiz questions to test your Unit 2 mastery.

## What to learn next

Continue to **Unit 3** with **B2-11-cultural-nuances**.
