---
{
  "id": "a1-05-review-unit1",
  "slug": "a1-05-review-unit1",
  "titleEn": "Review Unit 1",
  "titleVi": "Ôn tập Unit 1",
  "subtitleEn": "Review greetings, numbers, daily verbs, and family",
  "subtitleVi": "Ôn tập chào hỏi, số đếm, động từ hàng ngày và gia đình",
  "level": "A1",
  "unit": 1,
  "skill": "review",
  "order": 5,
  "cefr": "A1",
  "minutes": 10,
  "tags": [
    "review",
    "unit1",
    "all-skills"
  ],
  "audioScript": "Hello! My name is Mai. I am a student.\nI have one sister and one brother.\nI wake up at six. I eat breakfast at seven.\nMy phone number is zero nine one, two three four, five six seven eight.\n",
  "listeningEnabled": true,
  "flashcardCount": 10,
  "quiz": [
    {
      "type": "choice",
      "prompt": "How do you introduce yourself in English?",
      "options": [
        "My name is...",
        "Me name is...",
        "I name is..."
      ],
      "answer": "My name is..."
    },
    {
      "type": "fill",
      "prompt": "My phone number is zero nine one, five six seven, eight nine zero. Write the number:",
      "answer": "091567890"
    },
    {
      "type": "choice",
      "prompt": "Which verb is correct for \"ăn\"?",
      "options": [
        "eat",
        "drink",
        "sleep"
      ],
      "answer": "eat"
    },
    {
      "type": "fill",
      "prompt": "This is my ___. She is my mom. (mẹ)",
      "answer": "mother"
    },
    {
      "type": "choice",
      "prompt": "What time do you wake up? (thức dậy)",
      "options": [
        "I sleep.",
        "I eat.",
        "I wake up."
      ],
      "answer": "I wake up."
    }
  ]
}
---

# Review Unit 1

## What did you learn?

In Unit 1, you learned four important lessons:

1. **Greetings** — How to say hello and introduce yourself
2. **Numbers** — How to count and say phone numbers
3. **Daily Verbs** — Common verbs for everyday activities
4. **Family** — Words for family members

## Quick review

### Greetings

| English              | Vietnamese              |
| -------------------- | ----------------------- |
| Hello, my name is... | Xin chào, tôi tên là... |
| Nice to meet you.    | Rất vui được gặp bạn.   |
| Good morning.        | Chào buổi sáng.         |

### Numbers 1-10

| Number | English |
| ------ | ------- |
| 1      | one     |
| 2      | two     |
| 3      | three   |
| 4      | four    |
| 5      | five    |
| 6      | six     |
| 7      | seven   |
| 8      | eight   |
| 9      | nine    |
| 10     | ten     |

### Daily Verbs

| English | Vietnamese |
| ------- | ---------- |
| wake up | thức dậy   |
| eat     | ăn         |
| drink   | uống       |
| work    | làm việc   |
| sleep   | ngủ        |

### Family

| English | Vietnamese |
| ------- | ---------- |
| mother  | mẹ         |
| father  | bố         |
| sister  | chị gái    |
| brother | anh trai   |

## Key patterns

> **I have**: `I have one sister.` (Tôi có một chị gái.)
>
> **This is**: `This is my mother.` (Đây là mẹ tôi.)
>
> **My phone number is**: `My phone number is 091 567 890.`

## Practice exercises

### Exercise 1: Fill in the blank

1. `Hello, my ___ is Mai.`
2. `I ___ up at six in the morning.`
3. `I have two ___ and one sister.`

### Exercise 2: Match the pairs

- mother — mẹ
- wake up — thức dậy
- five — năm
- Nice to meet you — Rất vui được gặp bạn

## What to learn next

In **A1-06-time-place**, you will learn words for time (morning, afternoon) and places (school, home, office).
