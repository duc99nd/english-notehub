...
Using the wrong verb form (V-ing vs. to-V) after verbs like _avoid_, _recommend_, _decide_, or _plan_ is a common mistake for non-native speakers. This document structures these rules with examples tailored to software development.
...
- **10 Practical Examples:**
  1.  **"I recommend using a composite index on these fields."** (Not: _recommend to use_)
  2.  **"We suggest creating a staging database first."** (Not: _suggest to create_)
  3.  **"We should avoid query nesting to keep response times low."** (Not: _avoid to nest_)
...
- **10 Practical Examples:**
  ...
  6.  **"This helper function is dedicated to parsing JSON logs."** (Note: _to_ here is a preposition, so V-ing follows).
...
- **10 Practical Examples:**
  1.  **"We decided to migrate our system to Google Cloud."** (Not: _decided migrating_)
