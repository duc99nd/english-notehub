---
{
  'id': 'a2-15-review-unit3',
  'slug': 'a2-15-review-unit3',
  'titleEn': 'Review Unit 3',
  'titleVi': 'Ôn tập Unit 3',
  'subtitleEn': 'Consolidate small talk, opinions, experiences, and reading',
  'subtitleVi': 'Củng cố small talk, ý kiến, kinh nghiệm và đọc hiểu',
  'level': 'A2',
  'unit': 3,
  'skill': 'review',
  'order': 15,
  'cefr': 'A2',
  'minutes': 10,
  'tags': ['review', 'unit3', 'small-talk', 'opinions', 'experiences', 'A2'],
  'audioScript': "Let's review Unit 3.\nYou can make small talk in English.\nYou can share your opinions politely.\nYou can talk about your life experiences.\nYou can read short personal stories.\n",
  'listeningEnabled': true,
  'flashcardCount': 20,
  'quiz':
    [
      {
        'type': 'choice',
        'prompt': 'Which phrase is a small talk starter?',
        'options':
          [
            'Please give me the file.',
            "Lovely weather today, isn't it?",
            'Where is the train station?',
          ],
        'answer': "Lovely weather today, isn't it?",
      },
      { 'type': 'fill', 'prompt': 'In my ___, we should wait. (opinion)', 'answer': 'opinion' },
      {
        'type': 'choice',
        'prompt': 'I have visited Tokyo. Which tense?',
        'options': ['Past simple', 'Present perfect', 'Future'],
        'answer': 'Present perfect',
      },
      { 'type': 'fill', 'prompt': 'She has worked here ___ 2018.', 'answer': 'since' },
      {
        'type': 'choice',
        'prompt': 'Which is NOT an opinion phrase?',
        'options': ['I believe', 'I think', 'I order'],
        'answer': 'I order',
      },
    ],
}
---

# Review Unit 3

## What we learned

In Unit 3, you built everyday conversation skills:

1. **Small talk** — Opening friendly conversations
2. **Opinions** — Sharing your ideas politely
3. **Experiences** — Talking about your life using present perfect
4. **Short reading** — Understanding personal stories

## Quick review

### Small talk starters

| English                   | Vietnamese                 |
| ------------------------- | -------------------------- |
| Lovely weather, isn't it? | Thời tiết đẹp nhỉ?         |
| How was your weekend?     | Cuối tuần của bạn thế nào? |
| Do you come here often?   | Bạn hay đến đây không?     |

### Opinion phrases

| English              | Vietnamese                |
| -------------------- | ------------------------- |
| I think...           | Tôi nghĩ...               |
| I believe...         | Tôi tin rằng...           |
| In my opinion...     | Theo ý kiến của tôi...    |
| I agree / I disagree | Tôi đồng ý / không đồng ý |

### Present perfect pattern

| Form          | Example                        |
| ------------- | ------------------------------ |
| have/has + V3 | I have visited Japan.          |
| ever          | Have you ever tried sushi?     |
| never         | I have never seen snow.        |
| for + time    | I have lived here for 5 years. |
| since + time  | I have worked here since 2018. |

### Reading strategy

| Strategy             | Why                 |
| -------------------- | ------------------- |
| Look for known words | Builds confidence   |
| Find time markers    | Anchors events      |
| Identify verbs       | Understands actions |

## Practice

Answer the five quiz questions to test your Unit 3 mastery.

## What to learn next

Congratulations on completing the **A2 curriculum!** Continue to the **B1** level for intermediate English.
